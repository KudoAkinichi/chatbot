...
export function generateResponse(
  ...
  if (!disabilityData.type) {
    ...
    response += `**Recommended Jobs:**\n${recommendations.jobs.map((job) => `• ${job}`).join("\n")}\n\n`;

    response += `**Helpful Skills to Develop:**\n${recommendations.skills.map((skill) => `• ${skill}`).join("\n")}\n\n`;

    response += `**Resume Tips:**\n${recommendations.resumeTips.map((tip) => `• ${tip}`).join("\n")}\n\n`;

    response += `**Remember:**\n${recommendations.motivation[0]}\n\n`;

    response += "Would you prefer opportunities that are remote or on-site? (Just type 'remote' or 'on-site', or skip this question)";

    return { response, updatedData };
  }

  // Detect job preference
  ...
  if (inputLower.includes("more") || inputLower.includes("explore")) {
    ...
    response += `For roles like ${recommendations.jobs.join(", ")}, consider joining online communities or forums where professionals with similar needs share their experiences and job opportunities.\n\n`;

    response += "Is there anything specific about these roles you'd like to know more about?";
